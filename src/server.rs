...
///
/// Holds a cheaply-cloneable [`AgentOrchestrator`] handle (all `Arc`-backed).
///
/// The `rmcp` tool router is *not* stored on the struct: since rmcp 1.x,
/// `#[tool_handler]` resolves the router through the `Self::tool_router()`
/// associated function generated by `#[tool_router]`, so a per-instance field
/// would just be a dead clone.
#[derive(Clone)]
pub struct AgentMcp {
...
impl AgentMcp {
    ...
    async fn agent_prompt(
        ...
        Ok(CallToolResult::success(vec![ContentBlock::text(
            render_prompt(&response.provider, &response.text),
        )]))
    }
...
impl ServerHandler for AgentMcp {
    fn get_info(&self) -> ServerInfo {
        // `ServerInfo`/`Implementation` are `#[non_exhaustive]` in rmcp >= 1, so they
        // must be built through the constructors/with_* setters rather than a struct
        // literal. The advertised protocol version stays pinned at 2024-11-05 (what
        // this server has always sent) instead of tracking the SDK's LATEST — bumping
        // the negotiated version is a deliberate change, not an upgrade side effect.
