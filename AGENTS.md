...
Defaults: `CARGO_BUILD_JOBS=1`, `CARGO_INCREMENTAL=0` (shared-host OOM hygiene). Under `CI=true`,
the script also waits for free RAM and skips a redundant full `cargo build`. Details:
[docs/LOCAL_CHECKS.md](docs/LOCAL_CHECKS.md).
...
- Browser providers only today; API/self-hosted are planned (features are placeholders)
- `agent_status` / `agent_list_providers` report **measured** tri-state availability + evidence
  (`available` only after a recent successful request; no CDP browser ⇒ `unavailable`)
- `agent_parallel_prompt` is concurrent (`orchestrator::fan_out`) with per-provider deadlines
  (`OrchestratorConfig::timeout` / `max_concurrent` are applied)
- Consensus is still a placeholder heuristic (longest response / fixed `0.5` score)
...
- [docs/LOCAL_CHECKS.md](docs/LOCAL_CHECKS.md) — CI parity + shared-host C5 notes
